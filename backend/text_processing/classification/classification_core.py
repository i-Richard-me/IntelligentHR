...
class ClassificationResult(BaseModel):
    ...
    topic: List[str] = Field(
        ...,
        description="基于回复内容识别出的主题分类，根据预定义的主题分类列表中选择。",
    )
...
class ClassificationInput(BaseModel):
    ...
    labels: List[str] = Field(..., description="预设的分类标签列表")
...
3. 主题分类：根据回复的具体内容，将其归类到以下类别，并仅输出分类名称。以下是分类名称的列表：
