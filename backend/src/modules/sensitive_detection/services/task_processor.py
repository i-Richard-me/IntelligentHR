...
from modules.sensitive_detection.models.task import SensitiveDetectionTask, TaskStatus
from modules.sensitive_detection.workflows.sensitive_detection_workflow import SensitiveDetectionWorkflow
...
class TaskProcessor:
    ...
    def __init__(self):
        """初始化任务处理器"""
        self.queue = TaskQueue("sensitive")
        self.file_service = FileService()
        self.detector = SensitiveDetectionWorkflow()
        self._processing_tasks: Dict[str, asyncio.Task] = {}
        logger.info("敏感信息检测任务处理器初始化完成")
        ...
        try:
            ...
            try:
                ...
                with next(get_task_db()) as new_db:
                    current_task = new_db.query(SensitiveDetectionTask).get(task_id)
    ...
    async def cancel_task(self, task_id: str) -> bool:
        ...
        with next(get_task_db()) as db:
            try:
                task = db.query(SensitiveDetectionTask).get(task_id)
                if not task:
    ...
    async def _get_task(self, db: Session, task_id: str) -> Optional[SensitiveDetectionTask]:
        """获取任务信息"""
        return db.query(SensitiveDetectionTask).filter(
            SensitiveDetectionTask.task_id == task_id).first()

    async def _update_task_status(
            self, db: Session, task: SensitiveDetectionTask, status: TaskStatus
    ) -> None:
        """更新任务状态"""
        task.status = status
        db.commit()
    ...
    async def _process_task_content(self, db: Session, task: SensitiveDetectionTask) -> None:
        """处理任务内容"""
        ...
        def update_progress(completed_count: int):
            ...
            if completed_count == task.total_records or completed_count - last_update_count >= config.sensitive_detection.batch_size:
                try:
                    # 获取新的数据库会话和任务实例
                    with next(get_task_db()) as new_db:
                        current_task = new_db.query(
                            SensitiveDetectionTask).get(task.task_id)
        ...
        try:
            results = await self.detector.async_batch_detect(
                texts=texts,
                context=task.context,
                sensitive_types=task.sensitive_types,
                session_id=task.task_id,
                max_concurrency=config.sensitive_detection.max_concurrency,
            ...
            for i, result in enumerate(results):
                if result:  # 添加空值检查
                    ...
                    row_data['sensitive_detections'] = [detection.dict() for detection in result.detections]
                    combined_results.append(row_data)

            # 在新的数据库会话中保存结果
            with next(get_task_db()) as new_db:
                current_task = new_db.query(SensitiveDetectionTask).get(task.task_id)
                if current_task and current_task.status != TaskStatus.CANCELLED:
    ...
    async def _handle_task_error(
        ...
        """处理任务错误"""
        with next(get_task_db()) as new_db:
            task = new_db.query(SensitiveDetectionTask).get(task_id)
