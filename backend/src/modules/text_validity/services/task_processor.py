...
from modules.text_validity.models.task import ValidityTask, TaskStatus
from modules.text_validity.workflows.text_validity_workflow import TextValidityWorkflow
...
class TaskProcessor:
    ...
    def __init__(self):
        """初始化任务处理器"""
        self.queue = TaskQueue("validity")
        self.file_service = FileService()
        self.validator = TextValidityWorkflow()
        self._processing_tasks: Dict[str, asyncio.Task] = {}
        logger.info("有效性检测任务处理器初始化完成")
        ...
        try:
            ...
            try:
                ...
                with next(get_task_db()) as new_db:
                    current_task = new_db.query(ValidityTask).get(task_id)
    ...
    async def cancel_task(self, task_id: str) -> bool:
        ...
        with next(get_task_db()) as db:
            try:
                task = db.query(ValidityTask).get(task_id)
                if not task:
    ...
    async def _get_task(self, db: Session, task_id: str) -> Optional[ValidityTask]:
        """获取任务信息"""
        return db.query(ValidityTask).filter(ValidityTask.task_id == task_id).first()

    async def _update_task_status(
            self, db: Session, task: ValidityTask, status: TaskStatus
    ) -> None:
        """更新任务状态"""
        task.status = status
        db.commit()
    ...
    async def _process_task_content(self, db: Session, task: ValidityTask) -> None:
        """处理任务内容"""
        ...
        def update_progress(completed_count: int):
            ...
            if completed_count == task.total_records or completed_count - last_update_count >= config.text_validity.batch_size:
                try:
                    # 获取新的数据库会话和任务实例
                    with next(get_task_db()) as new_db:
                        current_task = new_db.query(
                            ValidityTask).get(task.task_id)
        ...
        try:
            results = await self.validator.async_batch_check(
                texts=texts,
                context=task.context,
                session_id=task.task_id,
                max_concurrency=config.text_validity.max_concurrency,
            ...
            # 在新的数据库会话中保存结果
            with next(get_task_db()) as new_db:
                current_task = new_db.query(ValidityTask).get(task.task_id)
                if current_task and current_task.status != TaskStatus.CANCELLED:
    ...
    async def _handle_task_error(
        ...
        """处理任务错误"""
        with next(get_task_db()) as new_db:
            task = new_db.query(ValidityTask).get(task_id)
