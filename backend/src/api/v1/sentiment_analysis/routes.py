...
import logging
from modules.sentiment_analysis.models import (
    TaskCreate,
    TaskResponse,
    TaskQueue,
    SentimentTask,
...
task_queue = TaskQueue("sentiment")
...
@router.get(
    ...
    response_model=TaskResponse,
    summary="获取任务状态",
    description="通过任务ID获取情感分析任务的当前状态"
)
async def get_task_status(
    ...
    """获取任务状态"""
    task = db.query(SentimentTask).filter(
        SentimentTask.task_id == task_id,
        SentimentTask.user_id == user_id
    ).first()
...
async def cancel_task(
    ...
    from modules.sentiment_analysis.services import TaskProcessor
...
async def list_tasks(
    ...
    """获取用户的所有任务"""
    tasks = db.query(SentimentTask).filter(
        SentimentTask.user_id == user_id
    ).order_by(SentimentTask.created_at.desc()).all()
...
@router.get(
    ...
    description="获取情感分析任务的结果文件的预签名下载URL"
)
