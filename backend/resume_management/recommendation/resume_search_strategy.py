...
HUMAN_MESSAGE_TEMPLATE_OVERALL = """
用户查询：
{refined_query}
...
resume_search_strategy_chain = LanguageModelChain(
    ResumeSearchStrategy,
    SYSTEM_MESSAGE_OVERALL,
    HUMAN_MESSAGE_TEMPLATE_OVERALL,
    language_model,
...
def generate_resume_search_strategy(state: Dict) -> Dict:
    """
    生成整体的简历搜索策略。
    ...
    Raises:
        ValueError: 如果缺少优化后的查询。
    """
    if not state.get("refined_query"):
        raise ValueError("缺少优化后的查询。无法生成搜索策略。")

    search_strategy_result = resume_search_strategy_chain.invoke(
        {"refined_query": state["refined_query"]},
    )

    resume_search_strategy = ResumeSearchStrategy(**search_strategy_result)

    state["collection_relevances"] = [
        {"collection_name": cr.collection_name, "relevance_score": cr.relevance_score}
...
# 生成集合特定搜索策略的系统消息模板
COLLECTION_SPECIFIC_SYSTEM_MESSAGE_TEMPLATE = """
...
COLLECTION_SPECIFIC_HUMAN_MESSAGE_TEMPLATE = """
用户查询：
{refined_query}
...
COLLECTION_SPECIFIC_INSTRUCTIONS = {
    "work_experiences": {
        "field_descriptions": """
...
def generate_collection_search_strategy(state: Dict) -> Dict:
    """
    为每个相关的集合生成详细的搜索策略。
    ...
    Returns:
        Dict: 更新后的状态字典，包含集合搜索策略。

    Raises:
        ValueError: 如果缺少优化后的查询或集合相关性。
    """
    if not state.get("refined_query") or not state.get("collection_relevances"):
        raise ValueError("缺少优化后的查询或集合相关性。无法生成集合搜索策略。")

    collection_search_strategies = {}

    for collection_info in state["collection_relevances"]:
        collection_name = collection_info["collection_name"]
        instructions = COLLECTION_SPECIFIC_INSTRUCTIONS.get(collection_name, {})

        if not instructions:
            print(f"警告：未找到 {collection_name} 的特定指令。跳过此集合的策略生成。")
            continue

        system_message = COLLECTION_SPECIFIC_SYSTEM_MESSAGE_TEMPLATE.format(
            collection_name=collection_name,
            field_descriptions=instructions["field_descriptions"],
            content_generation_guidelines=instructions["content_generation_guidelines"],
        )
        ...
        collection_search_strategies[collection_name] = search_strategy_result

    state["collection_search_strategies"] = collection_search_strategies
